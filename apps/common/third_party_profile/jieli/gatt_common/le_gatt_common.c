...
void ble_comm_set_config_name(const char *name_p, u8 add_ext_name)
{
    ...
    #endif
    u8 ext_name_len;
    #if 1 // 添加BLE MAC 最后两位(4Bytes)
        u8 ble_addr[6];
        le_controller_get_mac(ble_addr);
        ext_name_len = 4;
        if (ble_device_name_len <= BT_NAME_LEN_MAX - ext_name_len) 
        {
            char ble_addr_hex[4];
            ble_addr_hex[0] = (ble_addr[4] >> 4);
            ble_addr_hex[1] = (ble_addr[4] & 0xF);
            ble_addr_hex[2] = (ble_addr[5] >> 4);
            ble_addr_hex[3] = (ble_addr[5] & 0xF);
            for (u8 index = 0; index < 4; index++)
            {
                if (ble_addr_hex[index] > 9)
                    ble_addr_hex[index] = ble_addr_hex[index] -10 + 'A';
                else
                    ble_addr_hex[index] = ble_addr_hex[index] + '0';
            }
            memcpy(&ble_device_name[ble_device_name_len], ble_addr_hex, ext_name_len);
            ble_device_name_len += ext_name_len;
        }
    #endif
